...
from .economics import waste_cost, orchestration_overhead, fanout_rework, cost_attribution, cost_class, is_call_row, REPORTED, ESTIMATED, UNMETERED
...
def build_data(root:Path, config:dict|None=None):
    ...
    costs=[float(r.get('cost_usd',0) or 0)+float(r.get('ci_cost_usd',0) or 0)+float(r.get('human_cost_usd',0) or 0) for r in metrics]
    verified={r.get('task_id') for r in metrics if r.get('result')=='verified' or r.get('event')=='task_verified'}-{None}
    total=sum(costs); waste=waste_cost(metrics)
    attribution=cost_attribution(metrics)
    ...
    context_misses=sum(1 for r in metrics if r.get('event') in {'context_packet_miss','context_refetch'})
    context_packets=sum(1 for r in metrics if r.get('event')=='context_packet')
    conflicts=sum(1 for e in events if e.get('event') in {'merge_conflict','merge_conflict_resolution'})
    review_wait=[float(r.get('review_wait_ms',0) or 0)/1000 for r in metrics if r.get('review_wait_ms') is not None]
    shadow=[r for r in metrics if r.get('event')=='shadow_review']
    ...
    summary={'total_cost':total,'reported_cost':attribution[REPORTED]['cost'],'estimated_cost':attribution[ESTIMATED]['cost'],
        ...
        'orchestration_overhead':orchestration_overhead(metrics),'fanout_rework':fanout_rework(events),
    ...
    return {'summary':summary,'waste':waste,'by_role':role,'by_runtime':runtime,'policies':policy_rows,'trends':trends,
        'routes':build_route_stats(metrics,outcomes),'outcomes':outsum,'flaky':flaky_stats(metrics),
...
$('#events').innerHTML=D.events.slice().reverse().map(e=>`<div class="event"><span class="small">${esc(e.ts||'')}</span> <b>${esc(e.event||'')}</b><div class="small"><code>${esc(JSON.stringify(e).slice(0,600))}</code></div></div>`).join('');</script></main></body></html>'''
