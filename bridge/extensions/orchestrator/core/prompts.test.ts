...
describe("core/prompts.ts parsePlanResponse", () => {
	...
	test("parses a real `orchestrator.cli plan` JSON response captured from the Python CLI (fixtures/orchestrator-cli-plan-response.json)", () => {
		...
		// `python3 -m orchestrator.cli plan <runId> bugfix 5 medium` against a fresh, empty
		// `CODING_AGENT_ORCHESTRATOR_HOME`. The Python CLI emits many fields `PlanResponse` doesn't
		// need (`topology_recommendation`, `policy`, `features`, `feature_inventory`, ...) — this
		// pins that `parsePlanResponse` tolerates the extras and still extracts exactly the fields
		// the pipeline reads.
