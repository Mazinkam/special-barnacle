...
import { METHOD, buildAliasTable } from "./models.ts";
...
describe("recon tool boundary", () => {
	test("carries read-only tools and the configured model from planned recon to subprocess creation", async () => {
		...
		await orchestrator.dispatchParallel(process.cwd(), "run", tasks,
			{ implementation_fast: { model: "provider/recon-model", effort: "low" } }, {} as never, 0, {
			recordEvent: async () => {},
			runProcess: (opts) => orchestrator.runSubagentProcess({ ...opts, spawnChild: (_command, args) => {
